    ...
    <header className="bg-white border-b-4 border-stone-900">
      <div className="max-w-5xl mx-auto px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <nav className="flex flex-wrap items-center justify-center gap-2">
            {TABS.map(tab => (
              <Link
                key={tab.key}
                href={tab.href}
                className={`px-3 py-1.5 font-mono text-xs sm:text-sm font-bold uppercase transition border-2 ${
                  activeTab === tab.key
        ...
        <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
          ...
          <form action={signOut} className="w-full md:w-auto text-center">
            <button
              type="submit"
              className="w-full md:w-auto text-sm font-mono font-bold uppercase text-stone-900 px-4 py-2 border-2 border-stone-900 hover:bg-red-500 hover:text-white transition shadow-[2px_2px_0px_0px_#1c1917] active:shadow-none active:translate-y-[2px] active:translate-x-[2px]"
            >
              Terminate
            </button>
          </form>
